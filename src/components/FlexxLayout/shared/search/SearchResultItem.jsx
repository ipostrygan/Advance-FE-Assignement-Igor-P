...
const TitleWithAncestors = ({title, flexGrow = false, ancestors}) => {
  if (ancestors.length === 0)
    return <Title title={title} flexGrow={flexGrow} />;
    ...
    return (
      <div className='flex items-center gap-1.5'>
        {shortcut.map(sc => (
          <kbd
            key={sc}
            className='kbd flex items-center justify-center is-6 bs-6 rounded-sm text-sm bg-actionSelected'
          >
            {sc}
          </kbd>
        ))}
...
const EnterComponent = ({active}) => {
  return (
    active && (
      <i className='ri-corner-down-left-fill text-xl text-textSecondary' />
    )
  );
};
    ...
    return (
      <div
        ...
        className={classnames(
          'flex items-center justify-between gap-4 relative plb-2 pli-4 cursor-pointer',
          {
          ...
          {action.name &&
            (action.subtitle ? (
              <div className='flex flex-col grow'>
                ...
                {action.subtitle && (
                  <span className='text-[13px] leading-[1.538462] text-textSecondary'>
                    {action.subtitle}
                  </span>
                )}
              </div>
