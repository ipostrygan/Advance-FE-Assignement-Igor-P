...
export const HorizontalSubMenuContext = createContext({
  getItemProps: () => ({}),
});
...
const SubMenu = (props, ref) => {
  ...
  const getPlacement = (level, direction) => {
    if (level === 0) return 'bottom-start';
    return direction !== 'rtl' ? 'right-start' : 'left-start';
  };
  ...
  const {y, refs, floatingStyles, context} = useFloating({
    ...
    placement: getPlacement(level, dir.current),
    middleware: [
      offset({
