const slider = {
  MuiSlider: {
    ...
    styleOverrides: {
      root: ({ownerState}) => ({
        ...getSliderDimensions(ownerState?.orientation, ownerState?.size),
      }),
      thumb: ({ownerState}) => ({
...
const getSliderDimensions = (orientation, size) => {
  if (orientation === 'horizontal') {
    return {height: size !== 'small' ? 6 : 4};
  }
  return {width: size !== 'small' ? 6 : 4};
};
